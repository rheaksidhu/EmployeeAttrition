...
#measured on very different scales contribute equally and the preprocessing pipeline
#remains identical to prior versions as required
...
#fit the preprocessor on training data only so that encoding and scaling statistics
#are derived solely from xTrain and never leak information from xTest
preprocessor.fit(xTrain)

#transform both splits using statistics fitted on xTrain only; this mirrors what
#a Pipeline would do but allows us to pass a plain numpy array to GridSearchCV
#with a bare RandomForest rather than a pipeline, which is required so that
#CalibratedClassifierCV can wrap the forest without nesting Pipeline inside Pipeline
xTrainTransformed = preprocessor.transform(xTrain)
xTestTransformed = preprocessor.transform(xTest)

#recover feature names after OHE expansion so SHAP values can be mapped back to
#human-readable column names for the Oracle output and importance chart
...
#define the hyperparameter search space; recall is used as the scoring metric because
#missing an employee who will leave (false negative) is more costly than a false alarm
#the full grid is provided below; set fullGridSearch=True to run all 648 combinations
#which will take significantly longer but may find marginally better parameters
fullGridSearch = False

if fullGridSearch:
    #full grid: 3 x 3 x 3 x 3 x 2 x 3 = 648 combinations x 5 folds = 3240 fits
    paramGrid = {
    ...
    #reduced grid covering the most impactful parameters; retains the key trade-offs
    #between tree depth, leaf size and ensemble diversity without the full runtime cost
    paramGrid = {
...
#base RandomForest with balanced class weight to correct for the ~16% positive rate;
#GridSearchCV will replace these parameters with the best found during search
...
#GridSearchCV is fitted only on xTrainTransformed; the test set is never passed here
#which guarantees the held-out evaluation remains completely unbiased
print("\n--- Running GridSearchCV on training set only (this may take several minutes) ---")
...
gridSearch.fit(xTrainTransformed, yTrain)

print("\nBest hyperparameters found by GridSearchCV:")
print(gridSearch.best_params_)
print(f"Best cross-validated recall (training set): {gridSearch.best_score_:.4f}")

#extract the best estimator from the grid search to wrap in calibration;
#refit=True in GridSearchCV already re-trains on full xTrainTransformed but we
#re-wrap it in CalibratedClassifierCV to obtain honest probability estimates
...
#CalibratedClassifierCV with cv=5 uses cross-validation internally to produce
#probability estimates that better reflect true empirical frequencies; a raw
#RandomForest can be overconfident so calibration makes 70% predictions genuinely
#mean a ~70% chance of attrition rather than an uncalibrated score
...
print("\n--- Training calibrated model on full training set ---")
calibratedModel.fit(xTrainTransformed, yTrain)

#evaluate on the held-out test set for the first and only time; predictions are
#made here using both thresholds to compare operational trade-offs
testProbabilities = calibratedModel.predict_proba(xTestTransformed)[:, 1]

#compute metrics at threshold 0.5 (the conventional default decision boundary)
testPredicted05 = (testProbabilities >= 0.5).astype(int)
testAucRoc = roc_auc_score(yTest, testProbabilities)
testF1at05 = f1_score(yTest, testPredicted05, pos_label=1, zero_division=0)
testPrecisionAt05 = precision_score(yTest, testPredicted05, pos_label=1, zero_division=0)
testRecallAt05 = recall_score(yTest, testPredicted05, pos_label=1, zero_division=0)

#compute metrics at threshold 0.3; lowering the threshold increases recall at the
#cost of precision which is the preferred trade-off when missing attrition is costly
testPredicted03 = (testProbabilities >= 0.3).astype(int)
testF1at03 = f1_score(yTest, testPredicted03, pos_label=1, zero_division=0)
testPrecisionAt03 = precision_score(yTest, testPredicted03, pos_label=1, zero_division=0)
testRecallAt03 = recall_score(yTest, testPredicted03, pos_label=1, zero_division=0)

#print a clear comparison table so the operational impact of each threshold is visible
print("\n--- Threshold Comparison on Held-Out Test Set (positive class = Attrition Yes) ---")
print(f"{'Metric':<15} {'Threshold 0.5':>15} {'Threshold 0.3':>15}")
print("-" * 47)
print(f"{'AUC-ROC':<15} {testAucRoc:>15.4f} {'(same)':>15}")
print(f"{'F1-Score':<15} {testF1at05:>15.4f} {testF1at03:>15.4f}")
print(f"{'Precision':<15} {testPrecisionAt05:>15.4f} {testPrecisionAt03:>15.4f}")
print(f"{'Recall':<15} {testRecallAt05:>15.4f} {testRecallAt03:>15.4f}")

#use SHAP TreeExplainer to compute per-employee feature attributions on the test set;
#CalibratedClassifierCV wraps the base RandomForest so we extract the underlying forest
#from the first calibration fold; using one fold's estimator is sufficient for relative
#feature attribution because SHAP ranks are stable across folds for this dataset size
print("\n--- Computing SHAP values on test set (per-employee explanations) ---")

#extract the base RandomForest from the first calibrated classifier;
#TreeExplainer requires a native tree model and cannot wrap the isotonic calibrator
firstBaseEstimator = calibratedModel.calibrated_classifiers_[0].estimator

shapExplainer = shap.TreeExplainer(firstBaseEstimator)
#check_additivity=False is set because calibration shifts the output probabilities and
#the raw SHAP values from the base tree will not exactly sum to the calibrated prediction;
#SHAP is used here solely for relative feature attribution, not probability reconstruction
rawShapValues = shapExplainer.shap_values(xTestTransformed, check_additivity=False)

#newer versions of shap return a 3D array of shape (samples, features, classes)
#while older versions return a list; both cases index class 1 (attrition = Yes)
...
#map OHE-expanded feature names back to their original column name so that
#retention suggestions can be looked up against human-readable feature names;
...
#map original column names to retention suggestions so actionable guidance is
...
#identify test set indices where predicted probability exceeds 0.3 threshold;
#resetting the index ensures positional alignment between the probability array
#and the SHAP dataframe rows
...
for displayRank, empIdx in enumerate(flaggedIndices[:5], start=1):
    ...
    #retrieve this employee's SHAP values and aggregate by original feature name
    #so that OHE-expanded columns (e.g. OverTime_No, OverTime_Yes) are combined
    #into a single attribution for the original column before ranking
    ...
    #sort by absolute aggregated SHAP value descending so the most influential
    #features for this specific employee are ranked first
...
#compute mean absolute SHAP value per feature across the full test set to rank
#global feature importance; this is more reliable than impurity-based importance
#which can inflate the importance of high-cardinality features
...
#plot top 15 features by mean absolute SHAP value as a horizontal bar chart;
#SHAP-based importance is preferred here because it reflects the actual model
#output rather than tree-splitting statistics used in impurity-based importance
...
shapImportancePath = 'v3_sonnet_fix1_shap_importance.png'
plt.savefig(shapImportancePath, dpi=150)
plt.show()
plt.close()
print(f"SHAP importance chart saved to: {shapImportancePath}")

#compute confusion matrix at threshold 0.3 because that is the operational threshold
#used in the Oracle; showing the matrix at 0.5 would misrepresent real-world performance
